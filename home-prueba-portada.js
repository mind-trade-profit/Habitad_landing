/* ==========================================================================
   LANDING HABITAD — se planta en / desde el panel de la tienda.
...
(function () {
  ...
  var esperada = '/'.replace(/\/+$/, '').toLowerCase();
  if (aqui !== esperada) return;
  window.__landingHabitad = true;

  var ANCLA = '.js-home-sections-container';
...
const vendidosDe = p => parseInt(String(p.vend || '').replace(/\D/g, ''), 10) || 0;
...
function pintarPacks(){
  const pista = $('#packs');
  if (!pista) return;
  const items = PRODUCTOS.filter(p => p.necs.includes('packs')).sort(ordenCombos);
...
window.addEventListener('resize', () => requestAnimationFrame(ubicarCombos));
...
var catalogoVivo = (function () {
  ...
  const FRESCO = 60 * 1000;      // antes de un minuto no se vuelve a leer
...
if (catalogoVivo.aplicarGuardado()) pintarChips();
pintar(); pintarPacks(); recalcular();
setTimeout(() => catalogoVivo.refrescar(), 400);

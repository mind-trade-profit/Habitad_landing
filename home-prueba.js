...
(function () {
  ...
  if (ficha) {
    ...
    if (deMeta) {
      var casa = Object.prototype.hasOwnProperty.call(DESTINOS, '')
        ? '/' : '/catalogos/';
...
function abrirProductoDelAnuncio(ultimoIntento){
  ...
  const p = productoPorMango(productoDelAnuncio);
  if (p && (p.necs.includes('packs') || hayEn(p, segmento))){
    ...
    track('producto_desde_anuncio', {item_name: p.n, metodo: 'ficha'});
    return true;
  }
  if (!ultimoIntento) return false;

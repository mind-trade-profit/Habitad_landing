...
function pintar(){
    ...
    if (texto){
      ...
      g.innerHTML = '<div class="vacio"><strong>Nada con «' + limpio + '»</strong>' +
        (enOtro && segmento !== 'minorista'
          ...
          : 'Probá con otra palabra: el nombre del producto o lo que necesitás cuidar.') + '</div>';
    } else {
  ...
  g.scrollLeft = 0;
  if (typeof actualizarFlechasCat === 'function') actualizarFlechasCat();
  prepararTandas(segmento + '|' + texto + '|' + (cat ? cat.n : ''));
}

/* ══ CATÁLOGO EN CELULAR · de a tandas ══
   En celular la grilla es una lista hacia abajo (ver "CATÁLOGO EN CELULAR" en
   el CSS), y 151 tarjetas seguidas dejaban el resto de la pagina a treinta
   pantallas. Se ven de a 12 y el pie dice cuantos faltan. En escritorio las
   clases no hacen nada: ahi manda el carrusel.

   Cuantos se ven se conserva mientras no cambie el filtro -- segmento,
   categoria o busqueda --: la lectura en vivo de la tienda repinta la grilla
   a los pocos segundos, y eso no puede devolver al cliente a los primeros 12.

   var y no const: pintar() puede correr antes de que el guion llegue aca. */
var TANDA_CATALOGO = 12;
var catTandas = {mostrados: 12, filtro: undefined};

function prepararTandas(filtro){
  if (!catTandas) return;
  if (filtro !== catTandas.filtro){
    catTandas.filtro = filtro;
    catTandas.mostrados = TANDA_CATALOGO;
  }
  aplicarTandas();
}

function aplicarTandas(){
  const caja = $('#catMas'), g = $('#grilla');
  if (!caja || !g || !catTandas) return;
  const cartas = g.querySelectorAll('.card');
  const total = cartas.length;
  const vistos = Math.min(catTandas.mostrados, total);
  cartas.forEach((c, i) => c.classList.toggle('card--luego', i >= vistos));
  const quedan = total - vistos;
  caja.hidden = !total;
  $('#catMasCuenta').textContent = quedan
    ? 'Mostrando ' + vistos + ' de ' + total + ' productos'
    : (total > TANDA_CATALOGO ? 'Estás viendo los ' + total + ' productos' : '');
  const boton = $('#catMasBoton');
  boton.hidden = !quedan;
  boton.textContent = 'Ver ' + Math.min(TANDA_CATALOGO, quedan) +
    (Math.min(TANDA_CATALOGO, quedan) === 1 ? ' producto más' : ' productos más');
}

{
  const boton = $('#catMasBoton');
  if (boton) boton.onclick = () => {
    const antes = catTandas.mostrados;
    catTandas.mostrados += TANDA_CATALOGO;
    aplicarTandas();
    const cartas = $('#grilla').querySelectorAll('.card');
    track('catalogo_ver_mas', {mostrados: Math.min(catTandas.mostrados, cartas.length),
                               total: cartas.length});
    /* El foco pasa al primer producto nuevo, sin mover la pantalla: con
       teclado o lector de pantalla se sigue desde ahi y no desde el boton,
       que quedo mas abajo. */
    const titulo = cartas[antes] && cartas[antes].querySelector('h3');
    if (titulo){
...
function llenarVista(p){
  ...
  if (!envio.hidden) envio.textContent = 'Envío gratis en compras superiores a ' + ars(nubea ? nubea.umbralEnvio() : 60000);
...
function mensajeProgreso(subtotal, falta, proximo, pct){
  ...
  if (!proximo){
    ...
    /* Para el envio gratis va el texto que el cliente escribio en Nubea. */
    const deNubea = proximo.nombre === 'Envío gratis' && nubea && nubea.textoEnvio('falta', falta);
...
function recalcular(){
  ...
  if (reciente){
    ...
    const msg = $('#bfMsg');
    msg.textContent = (t.nombre === 'Envío gratis' && nubea && nubea.textoEnvio('logrado')) ||
...
var categoriasVivas = (function () {
  ...
  function pintarChips(delSegmento, buscando){
    ...
    if (firma !== firmaChips){
      firmaChips = firma;
      caja.innerHTML = '';
      caja.appendChild(boton('', 'Todos', delSegmento.length));
  ...
  /* El boton de celular solo tiene sentido si de verdad hay categorias
     tapadas. */
  function ajustarBoton(){
    const caja = $('#chips'), b = $('#chipsMas'), barra = $('#chipsBarra');
    if (!caja || !b || !barra) return;
    if (caja.hidden){ b.hidden = true; barra.classList.remove('abierta'); return; }
    const abierta = barra.classList.contains('abierta');
    const tapadas = !abierta && caja.scrollHeight > caja.clientHeight + 2;
    b.hidden = !(tapadas || abierta);
    b.textContent = abierta ? 'Ver menos' : 'Ver todas las categorías';
  }
  {
    const b = $('#chipsMas'), barra = $('#chipsBarra');
    if (b && barra) b.onclick = () => { barra.classList.toggle('abierta'); ajustarBoton(); };
    addEventListener('resize', ajustarBoton);
  }
  ...
  if (EN_TIENDA){
    // El admin cambia algo en otra pestaña y vuelve: se ve sin recargar.
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible') refrescar();
    });
  }

  const api = {enTienda: EN_TIENDA, informe: null, iniciar, refrescar, pintarChips, soltar,
               nombresDe, leerMenu, probar,
...
pintar(); pintarPacks(); recalcular();
setTimeout(() => {
  catalogoVivo.refrescar();
  // En paralelo, que tiene hoy cada categoria. Repinta sola si cambio algo.
  categoriasVivas.refrescar();
}, 400);

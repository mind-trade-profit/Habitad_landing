...
(function () {
  ...
  if (ficha) {
    ...
    var deMeta = /[?&]fbclid=/i.test(busca) ||
  ...
  if (!items.length){
    ...
    if (texto){
      ...
      g.innerHTML = '<div class="vacio"><strong>Nada con «' + limpio + '»</strong>' +
        ...
        '&tienda=1">Buscarlo en el catálogo completo de la tienda</a></div>';
    } else {
...
function abrirFichaDeMango(mango, conRespaldo){
  ...
  /* O no esta en la landing, o no se vende a este segmento. La busqueda por
     nombre lo muestra igual, y cuando es de otro segmento la grilla ya avisa
     que existe con precio de lista. */
  const puesta = ponerBusqueda(p ? p.n : nombreDeMango(mango), !!p);
  return {p, como: p ? 'otro-segmento' : puesta ? 'busqueda' : 'catalogo'};
}

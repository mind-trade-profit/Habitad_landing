...
(function () {
  ...
  alCargar(function () {
    ...
    var sueltos = caja.querySelectorAll('.modal, .barra-fija');
...
const TIERS_POR_SEGMENTO = {
  ...
  minorista: [
    {min:30000,  nombre:'2 cuotas sin interés',   real:true},
    {min:60000,  nombre:'Envío gratis',           real:true},
    /* A CONFIRMAR. Hoy la tienda da 5% por transferencia en todo el catalogo
       y sin minimo, asi que este hito promete algo distinto de lo que el
       checkout va a cobrar. Se marca hasta que el cliente lo defina. */
    {min:90000,  nombre:'6% en transferencia',    real:true, pendiente:true}
  ],
  ...
  distribuidor: [
    {min:250000, nombre:'Mínimo distribuidor',    real:true},
...
/* Los combos se venden a precio de lista en los tres segmentos -- asi quedo
   la seccion de packs -- por eso no se filtran por segmento. Si se filtraran,
   el chip diria 0 en mayorista mientras la seccion de abajo muestra 26. */
const esPack = need => need === 'packs';
const cuantos = (need, seg) =>
  PRODUCTOS.filter(p => p.necs.includes(need) && (esPack(need) || hayEn(p, seg))).length;
...
const EVENTOS = {
  ...
  select_category:      {ga4: 'select_content',   meta: null},
  search:               {ga4: 'search',           meta: 'Search'},
  search_result_open:   {ga4: 'select_item'},
...
/* ---- Carrusel de fondo del hero ----
   Los cuatro banners del inicio de la tienda. Para agregar o sacar uno,
   se toca solo este arreglo y el archivo en prototipo/img/. */
...
(function heroCarrusel(){
  const caja = $('#heroFondos');
  if (!caja || !FONDOS.length) return;

  FONDOS.forEach((f, i) => {
    const pic = document.createElement('picture');
    pic.className = 'hero__slide' + (i === 0 ? ' activa' : '');
    pic.innerHTML = '<source media="(max-width: 700px)" srcset="' + f.celular + '">' +
                    '<img src="' + f.escritorio + '" alt="" ' +
  ...
  function mostrar(i, manual){
    ...
    puntos.children[actual].setAttribute('aria-current', 'false');
    actual = (i + slides.length) % slides.length;
    slides[actual].classList.add('activa');
    puntos.children[actual].setAttribute('aria-current', 'true');
    ponerCta(actual);
    if (manual) arrancar();
  }
  ponerCta(0);
...
/* Las categorias se dibujan de nuevo cada vez que cambia el segmento, porque
   cambian las cantidades y con ellas el orden. Van de mayor a menor: primero
   lo que la tienda mas ofrece, y al final lo que no tiene nada. Antes el orden
   estaba fijo a mano y no decia nada. */
function pintarChips(){
  const caja = $('#chips');
  if (!caja) return;
  const conCantidad = NECESIDADES
    .map(n => Object.assign({}, n, {cant: cuantos(n.id, segmento)}))
    .sort((a, b) => (b.cant - a.cant) || a.label.localeCompare(b.label, 'es'));
  caja.innerHTML = '';
  /* Una categoria sin productos en este segmento no se muestra. Antes se
     mostraba apagada, y en distribuidor eso dejaba "Aromas y ambiente: 0" en
     pantalla: una categoria que existe pero no se puede comprar. */
  conCantidad.filter(n => n.cant > 0).forEach(n => {
    const b = document.createElement('button');
    b.className = 'chip' + (n.destacado ? ' chip--packs' : '');
    b.type = 'button'; b.dataset.need = n.id;
    b.setAttribute('aria-pressed', n.id === activa);
    b.innerHTML = n.label + '<span class="n">' + n.cant + '</span>';
    b.onclick = () => filtrar(n.id);
    caja.appendChild(b);
  });
  /* Si la categoria abierta se quedo sin productos al cambiar de segmento,
     hay que mover al cliente a una que tenga: si no, queda mirando una grilla
     vacia sin ningun chip marcado. */
  const viva = conCantidad.find(n => n.cant > 0 && n.id === activa);
  if (!viva){
    const primera = conCantidad.find(n => n.cant > 0);
    if (primera){
      activa = primera.id;
      caja.querySelectorAll('.chip').forEach(b =>
        b.setAttribute('aria-pressed', b.dataset.need === activa));
    }
  }
  ajustarBotonChips();
}
/* El boton solo tiene sentido si de verdad hay categorias tapadas. */
function ajustarBotonChips(){
  const caja = $('#chips'), boton = $('#chipsMas'), barra = $('#chipsBarra');
  if (!caja || !boton || !barra) return;
  const abierta = barra.classList.contains('abierta');
  const tapadas = !abierta && caja.scrollHeight > caja.clientHeight + 2;
  boton.hidden = !(tapadas || abierta);
  boton.textContent = abierta ? 'Ver menos' : 'Ver todas las categorías';
}

{
  const boton = $('#chipsMas'), barra = $('#chipsBarra');
  if (boton && barra) boton.onclick = () => {
    barra.classList.toggle('abierta');
    ajustarBotonChips();
  };
  addEventListener('resize', ajustarBotonChips);
}
...
function pedirAscenso(destino){
  ...
  const viajan = items.filter(i => hayEn(i.prod, destino));
  const quedan = items.filter(i => !hayEn(i.prod, destino));
  ...
  if (falta){
    ...
    return pintarAviso({
      ...
      texto: 'Para comprar a precio <b>' + nombre + '</b> te faltan <b>' +
        ...
        ars(MINIMOS[destino]) + '</b>.<br><br>Seguí sumando y volvé a tocar ' +
...
function convertirCarrito(destino){
  Object.entries(carrito).forEach(([k, i]) => {
    if (!hayEn(i.prod, destino)) delete carrito[k];
    else i.seg = destino;
  });
}
...
function aplicarSegmento(id, conservarCarrito){
  ...
  logradosPrevios = 0;
  pintarHitos();
  // Los chips se rehacen: cambian las cantidades y tambien el orden.
  pintarChips();
  // El envio gratis es un beneficio minorista: al mayorista y al distribuidor
  // no se les promete algo que no van a tener.
  const beneEnvio = $('#beneEnvio');
  if (beneEnvio) beneEnvio.textContent = id === 'minorista'
    ? 'Gratis superando $60.000' : 'A cargo del comprador';
...
function tarjeta(p, key){
  ...
  el.className = 'card js-item-product';
  ...
  el.innerHTML =
    '<div class="foto">' +
      ...
      (sinStock ? '<span class="tag tag--bug">sin stock</span>' : '') + '</div>' +
    '<div class="card-body">' +
      '<h3>' + p.n + '</h3>' +
      // Hueco donde la app deja las estrellas. Vacio no ocupa lugar.
      estrellasDe(p) +
      '<p class="desc">' + resumenTarjeta(p) + '</p>' +
      (p.vars ? '<div class="pres">' + (p.opcion || 'Variante') + '</div>' +
        '<select class="variante-sel" data-variante>' + p.vars.map((v, i) =>
...
function pintar(){
  ...
  const need = NECESIDADES.find(n => n.id === activa);
  const items = PRODUCTOS.filter(p => p.necs.includes(activa) &&
    (esPack(activa) || hayEn(p, segmento)));
  const seg = SEGMENTOS[segmento].label.toLowerCase();
  $('#cat-sub').textContent = items.length
    ? items.length + ' producto(s) con precio ' + seg + ' en ' + need.label + '.' : '';
  if (!items.length){
    // Vacia porque la tienda no publica ese rubro para ese segmento, no por un
    // error de codigo. Se dice cual es la situacion real.
    const enMino = cuantos(activa, 'minorista');
    g.innerHTML = '<div class="vacio"><strong>No hay productos ' + seg + ' en ' +
      need.label + '</strong>' + (enMino
        ? 'La tienda tiene ' + enMino + ' con precio minorista, pero ninguna publicacion ' +
          seg + ' para este rubro. Se resuelve publicandolos desde el admin.'
        : 'Todavia no hay productos cargados en esta necesidad.') + '</div>';
    return;
  }
  items.forEach((p, i) => g.appendChild(tarjeta(p, activa + '-' + i)));
  // Cada necesidad arranca desde el principio, no donde quedó la anterior.
  g.scrollLeft = 0;
...
function filtrar(id){
  activa = id;
  document.querySelectorAll('.chip').forEach(c => c.setAttribute('aria-pressed', c.dataset.need === id));
  history.replaceState(null, '', '#necesidades?n=' + id);
  const need = NECESIDADES.find(n => n.id === id);
  track('select_category', {need_id:id, need_name:need.label});
  pintar(); recalcular();
}
...
/* Casi todas las descripciones arrancan repitiendo el nombre del producto, y
   en el panel eso deja el titulo dos veces seguidas. Se compara sin acentos ni
   puntuacion, porque la repeticion no siempre viene escrita igual
   ("ANTIAGE-LINEAS" en el nombre y "ANTIAGE - LINEAS" en la descripcion). */
function descPack(p){
  const d = (p.d || '').replace(/\s+/g, ' ').trim();
  if (!d) return '';
  const clave = t => t.toLowerCase().normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, '');
  const limpiar = t => t.replace(/^[\s:\-\u2013.,]+/, '').trim();

  /* Dos pasadas, porque la tienda repite el titulo de dos maneras distintas y
     ninguna sola las cubre:

     1) Mismo texto con otra puntuacion o acentos -- "ANTIAGE-LINEAS" arriba y
        "ANTIAGE - LINEAS" abajo. Se compara caracter a caracter ignorando todo
        lo que no sea letra o numero.
     2) Titulo parecido pero no igual -- "COMBO ACEITE PARA TU CUIDADO DE PIEL"
        contra "Combo Aceite Cuidado de Piel". Ahi se descartan las palabras
        iniciales que ya aparecen en el titulo. */
  const n = clave(p.n);
  if (n){
    let i = 0, visto = '';
    while (i < d.length && visto.length < n.length){ visto += clave(d[i]); i++; }
    if (visto === n) return limpiar(d.slice(i));
  }

  const titulo = new Set(p.n.split(/\s+/).map(clave).filter(Boolean));
  if (!titulo.size) return d;
  const palabras = d.split(' ');
  let k = 0;
  while (k < palabras.length && titulo.has(clave(palabras[k]))) k++;
  // Frenos: una sola palabra en comun puede ser casualidad, y la descripcion
  // nunca se puede quedar sin contenido.
  if (k < 2 || k > palabras.length - 4) return d;
  return limpiar(palabras.slice(k).join(' '));
}

/* Tarjeta de pack. No reusa tarjeta() porque el orden es otro: la foto manda
   y el panel se le monta encima. Las acciones y el carrito son los mismos. */
function tarjetaPack(p, key){
  const pr = precioDe(p), lista = p.pr.minorista;
  const rebaja = (segmento !== 'minorista' && lista && pr && lista > pr)
    ? Math.round((1 - pr / lista) * 100) : 0;
  const sinStock = !conStock(p);
  const el = document.createElement('article');
  el.className = 'card card--pack js-item-product';
  const idTienda = p.ids.minorista || p.ids.mayorista || p.ids.distribuidor;
  if (idTienda) el.setAttribute('data-store', 'product-item-' + idTienda);
  el.innerHTML =
    '<div class="foto">' +
    ...
    '<div class="pack-panel">' +
      '<h3>' + nombrePack(p.n) + '</h3>' +
      '<p class="desc">' + descPack(p) + '</p>' +
      estrellasDe(p) +
      (p.vars ? '<select class="variante-sel" data-variante>' + p.vars.map((v, i) =>
        '<option value="' + i + '"' + (v.s ? '' : ' disabled') + '>' + v.n +
        (v.s ? '' : ' (sin stock)') + '</option>').join('') + '</select>' : '') +
      '<div class="pack-precio"><b class="precio">' + ars(pr) + '</b>' +
        (rebaja ? '<s>' + ars(lista) + '</s>' : '') + '</div>' +
    '</div>' +
    '<div class="card-body">' +
      '<div class="acciones">' +
        '<div class="qty"><button type="button" data-d="-1">-</button>' +
...
function pintarPacks(){
  const caja = $('#packs');
  if (!caja) return;
  /* Los combos se muestran siempre, en los tres segmentos. La tienda no los
     publica con precio mayorista ni distribuidor, asi que se venden al precio
     de lista y se dice: es preferible eso a esconderlos. */
  const items = PRODUCTOS.filter(p => p.necs.includes('packs'));
  const aPrecioDeLista = segmento !== 'minorista' && !items.some(p => hayEn(p, segmento));
  const sub = caja.closest('.bloque').querySelector('.sub');
  if (!items.length){
    // Igual que el catalogo: si esta vacio se dice por que, en vez de dejar
    // el hueco mudo que es lo que pasaba hasta ahora.
    caja.innerHTML = '<div class="vacio"><strong>No hay packs con precio ' +
      SEGMENTOS[segmento].label.toLowerCase() + '</strong>La tienda todavía no publica ' +
      'los combos para este segmento. Se resuelve publicándolos desde el admin.</div>';
    if (sub) sub.textContent = '';
    return;
  }
  caja.innerHTML = '';
  items.forEach((p, i) => caja.appendChild(tarjetaPack(p, 'pack-' + i)));
  caja.scrollLeft = 0;
...
function flechasPacks(){
  const pista = $('#packs');
  if (!pista) return;
  const tope = pista.scrollWidth - pista.clientWidth - 2;
  const izq = $('[data-pack="-1"]'), der = $('[data-pack="1"]');
  if (izq) izq.disabled = pista.scrollLeft <= 2;
  if (der) der.disabled = pista.scrollLeft >= tope;
}

document.querySelectorAll('[data-pack]').forEach(b => b.addEventListener('click', () => {
  const pista = $('#packs');
  pista.scrollBy({left: pista.clientWidth * Number(b.dataset.pack), behavior: 'smooth'});
  requestAnimationFrame(flechasPacks);
}));
{
  const pista = $('#packs');
  if (pista) pista.addEventListener('scroll', flechasPacks, {passive: true});
}
window.addEventListener('resize', flechasPacks);
...
function actualizarFlechasCat(){
  ...
  if (pistaCatAyuda){
    const conFlechas = flechaCatDer && getComputedStyle(flechaCatDer).display !== 'none';
...
if (pistaCat) pistaCat.addEventListener('scroll', actualizarFlechasCat, {passive:true});
...
function abrirVista(p, key){
  ...
  /* Descripciones de hasta 2.600 caracteres: un muro de texto que nadie lee y
     que empuja el boton de compra fuera de pantalla. Se muestra el arranque y
  ...
  // Envio gratis: se anuncia solo al minorista, que es el unico que lo tiene.
  const envio = $('#mvEnvio');
  envio.hidden = segmento !== 'minorista';
  if (!envio.hidden) envio.textContent = 'Envío gratis en compras superiores a ' + ars(60000);
...
function pintarHitos(){
  ...
  // La misma escalera, en fila, para celular.
  const fila = $('#tlEtiquetas');
  if (fila) fila.innerHTML = TIERS.map(t => '<span>' + etiqueta(t) + '</span>').join('');
}
pintarHitos();
...
function mensajeProgreso(subtotal, falta, proximo, pct){
  ...
  if (!proximo){
    ...
    msg.innerHTML = (falta < 12000 ? '¡Ya estás cerca! ' : '') +
...
function recalcular(){
  ...
  if (reciente){
    ...
    const msg = $('#bfMsg');
    msg.textContent = '¡' + t.nombre.toUpperCase() + ' DESBLOQUEADO!';
...
function sugerir(subtotal, falta, proximo){
  ...
  if (proximo && subtotal > 0){
    ...
    const cand = PRODUCTOS.filter(p => hayEn(p, segmento) && conStock(p) && !enCarrito.has(p.n))
      ...
      .sort((a,b) => (b.necs.includes(activa)) - (a.necs.includes(activa))).slice(0,3);
    if (cand.length){
...
function resumenCarrito(){
  ...
  /* El envio no se inventa: el costo lo calcula Tiendanube en el checkout con
     ...
     es si aplica el envio gratis minorista. */
  const gratisDesde = 60000;
  $('#checkoutEnvio').textContent = segmento !== 'minorista'
    ? 'A cargo del comprador'
    : (total >= gratisDesde ? '\u00a1Gratis!' : 'Se calcula al finalizar');
...
/* ══════════ BUSCADOR ══════════
   Busca sobre el mismo catalogo que muestra la landing y abre NUESTRA ficha de
   producto, no la del theme. Ese es el punto: si el cliente busca por el
   buscador de Brasilia se va a la ficha vieja y sale del embudo.

   Compara sin acentos y por palabras sueltas, asi "aceite almendra" encuentra
   "Aceite de Almendras" y "almendras" tambien encuentra "ALMENDRA". */
(function buscador(){
  const campo = $('#buscar'), lista = $('#buscarLista'), limpiar = $('#buscarLimpiar');
  if (!campo || !lista) return;

  const clave = t => (t || '').toLowerCase().normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim();

  /* Raiz de una palabra, para que el singular y el plural se encuentren.
     Sin esto "geles" no devolvia nada -- ni "cremas", ni "aceites" -- porque
     la comparacion era por texto exacto y en el catalogo dice "Gel" y
     "Crema". Se corta el plural castellano: -es despues de consonante
     ("geles" -> "gel") y -s despues de vocal ("cremas" -> "crema").
     Palabras de tres letras o menos no se tocan: "mas" no es plural de "ma". */
  const raiz = w => {
  ...
  /* Cada producto se indexa por las raices de su nombre Y de su categoria,
     asi una palabra generica como "gel" o "aceite" cae en todo lo que
     corresponde aunque el nombre lo escriba en plural. */
  const raicesDe = p => {
    if (p._raices) return p._raices;
    const cats = p.necs.map(id => (NECESIDADES.find(n => n.id === id) || {}).label || '');
    p._raices = clave(p.n + ' ' + cats.join(' ')).split(' ').map(raiz).filter(Boolean);
    return p._raices;
  };
  const contieneRaiz = (p, w) => {
    const r = raiz(w);
    /* Las dos raices tienen que medir al menos tres letras para compararse por
       prefijo. Sin ese piso, buscar "gel" tambien devolvia todo lo que dice
       "100 g" en el nombre -- que son decenas -- porque "gel" empieza con "g".
       Medido: 12 resultados donde correspondian 7. */
  ...
  function buscar(texto){
    const palabras = clave(texto).split(' ').filter(p => p.length > 1);
    if (!palabras.length) return [];
    // Solo lo que el cliente puede comprar en el segmento que esta mirando,
    // igual que el resto de la landing. Los combos valen en los tres.
    const candidatos = PRODUCTOS.filter(p =>
      p.necs.includes('packs') || hayEn(p, segmento));
    return candidatos
      .map(p => {
        const nombre = clave(p.n);
        // Primero el texto tal cual; si no da, por raices, que es lo que hace
        // que "geles" encuentre los geles.
        const literal = palabras.every(w => nombre.includes(w));
        if (!literal && !palabras.every(w => contieneRaiz(p, w))) return null;
        // Ordena mejor lo que empieza con lo buscado que lo que solo lo
        // contiene, y lo literal antes que lo encontrado por raiz.
        const arranca = nombre.startsWith(palabras[0]) ? 0 : 1;
        return {p, orden: (literal ? 0 : 4000) + arranca * 1000 + nombre.length};
      })
      .filter(Boolean)
  ...
  /* La busqueda se mide cuando el cliente para de escribir, no en cada
     tecla: si no, "aceite" son seis eventos y el informe queda ilegible. */
  let relojBusqueda = null;
  function medirBusqueda(texto){
    clearTimeout(relojBusqueda);
    if (!texto.trim()) return;
    relojBusqueda = setTimeout(() => {
      // search_term es el nombre que espera GA4 en su evento `search`;
      // query queda para el dataLayer, que es como se venia llamando.
      track('search', {search_term: texto.trim(), query: texto.trim(),
                       results: resultados.length});
    }, 900);
  }

  /* Veinte y no ocho: una palabra generica como "aceite" o "tintura" da mas
     de veinte resultados, y cortarlos en ocho escondia media categoria. La
     lista ya tiene su propio scroll, asi que no crece la pagina. */
  const TOPE_RESULTADOS = 20;

  function pintar(texto){
    const todos = buscar(texto);
    const sobran = Math.max(0, todos.length - TOPE_RESULTADOS);
    resultados = todos.slice(0, TOPE_RESULTADOS);
    medirBusqueda(texto);
    marcado = -1;
    limpiar.hidden = !texto;
    if (!texto.trim()){
      lista.hidden = true; campo.setAttribute('aria-expanded', 'false'); return;
    }
    if (!resultados.length){
      // Si no hay nada acá pero sí con precio de lista, se dice: es la
      // diferencia entre "no existe" y "no lo vendemos a este segmento".
      const enLista = PRODUCTOS.filter(p => {
        const palabras = clave(texto).split(' ').filter(w => w.length > 1);
        return palabras.length && palabras.every(w => contieneRaiz(p, w));
      }).length;
      lista.innerHTML = '<p class="buscador__nada">Nada con <b>' +
        texto.replace(/[<>&]/g, '') + '</b>' +
        (enLista ? ' con precio ' + SEGMENTOS[segmento].label.toLowerCase() +
                   '. Hay ' + enLista + ' con precio de lista.' : ' en el catálogo.') +
        '</p>';
      lista.hidden = false; campo.setAttribute('aria-expanded', 'true'); return;
    }
    lista.innerHTML = resultados.map((p, i) => {
      const nec = (NECESIDADES.find(n => p.necs.includes(n.id)) || {}).label || '';
      return '<button type="button" class="buscador__item" role="option" data-i="' + i + '">' +
        (p.img ? '<img class="buscador__foto" src="' + p.img + '" alt="" loading="lazy">'
               : '<span class="buscador__foto"></span>') +
        '<span class="buscador__txt"><span class="buscador__nom">' + p.n + '</span>' +
        '<span class="buscador__cat">' + nec + '</span></span>' +
        '<span class="buscador__precio">' + ars(precioDe(p)) + '</span></button>';
    }).join('');
    if (sobran){
      lista.insertAdjacentHTML('beforeend',
        '<p class="buscador__nada">y ' + sobran + ' más. Escribí algo más ' +
        'específico para achicar la lista.</p>');
    }
    lista.querySelectorAll('.buscador__item').forEach(b =>
      b.onclick = () => abrir(+b.dataset.i));
    lista.hidden = false;
  ...
  function abrir(i){
    ...
    if (!p) return;
    track('search_result_open', {item_name: p.n, query: campo.value});
    cerrar();
    abrirVista(p, 'busqueda-' + p.n);
  }
  ...
  function marcar(paso){
    ...
    const items = lista.querySelectorAll('.buscador__item');
    if (marcado >= 0) items[marcado].setAttribute('aria-selected', 'false');
  ...
  // Se espera a que deje de tipear: buscar en cada tecla sobre 145 productos
  // no hace falta y ademas parpadea.
  let reloj = null;
  campo.addEventListener('input', () => {
    clearTimeout(reloj);
    reloj = setTimeout(() => pintar(campo.value), 140);
  });

  campo.addEventListener('keydown', e => {
    if (e.key === 'ArrowDown'){ e.preventDefault(); marcar(1); }
    else if (e.key === 'ArrowUp'){ e.preventDefault(); marcar(-1); }
    else if (e.key === 'Enter'){ e.preventDefault(); abrir(marcado >= 0 ? marcado : 0); }
    else if (e.key === 'Escape'){ cerrar(); campo.blur(); }
  });

  limpiar.onclick = () => { campo.value = ''; pintar(''); campo.focus(); };

  /* Busqueda que llega por la URL: /?q=calendula
     Es la pieza que permite conectar el buscador del theme con este sin tocar
     el de la tienda. En vez de deshabilitar el buscador de Brasilia -- que
     dejaria al cliente sin buscar en el resto del sitio -- alcanza con que su
     pagina de resultados redirija aca con ?q=, y el cliente cae en NUESTRA
     ficha en vez de en la del theme. */
  {
  ...
  // Al cambiar de segmento cambian precios y disponibilidad.
...
async function mandarALaTienda(items, boton, textoOriginal){
  ...
  const sinPanel = () => {};   // reemplaza la apertura de la gaveta
  let fallaron = 0;
  for (const it of items){
    const f = formularioParaLaTienda(it.prod, it.cant, it.seg, it.vi);
    try {
      await LS.addToCartEnhanced(f, '', '', '', true, sinPanel);
    } catch (err) {
      fallaron++;
    } finally {
      f.remove();

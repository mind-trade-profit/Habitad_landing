...
function tarjeta(p, key){
  ...
  el.innerHTML =
    ...
    '<div class="card-body">' +
      ...
      (function(){ const b = nubea ? nubea.badgesHtml(p, 'tarjeta') : '';
                   return b ? '<div class="nb-badges">' + b + '</div>' : ''; })() +
      '<div class="acciones">' +
...
/* ---- Catalogo ----
   Sin categorias, pedido del cliente: se ve todo lo que se puede comprar en el
   segmento elegido, y quien quiere achicar escribe en el buscador. Los combos
   entran en los tres segmentos, a precio de lista, igual que en su carrusel.
...
function pintar(){
  ...
  const items = texto
    ...
    : PRODUCTOS.filter(p => p.necs.includes('packs') || hayEn(p, segmento)).sort(ordenDestacados);

  $('#cat-sub').textContent = !items.length ? '' : texto
    ? items.length + (items.length === 1 ? ' resultado' : ' resultados') +
      ' para «' + texto + '» con precio ' + seg + '.'
    : items.length + ' productos con precio ' + seg + '.';
...
/* ══════════ BUSCADOR ══════════
   Es el unico filtro del catalogo desde que salieron las categorias: el
   cliente ve todo y achica escribiendo. Filtra la grilla en el momento, y la
   ficha que se abre es la de ESTA landing, no la del theme.

   Compara sin acentos y por palabras sueltas, asi "aceite almendra" encuentra
   "Aceite de Almendras". Y busca tambien por la necesidad de cada producto:
   "dolor" o "piel" traen lo que antes estaba en esa categoria. */
...
/* Raices del nombre Y de las necesidades de cada producto. Se guardan, y se
   rehacen si el catalogo vivo le cambio el nombre. */
function raicesDe(p){
  if (p._raices && p._raicesDe === p.n) return p._raices;
  const cats = p.necs.map(id => (NECESIDADES.find(n => n.id === id) || {}).label || '');
  p._raices = claveBusqueda(p.n + ' ' + cats.join(' ')).split(' ').map(raizBusqueda).filter(Boolean);
  p._raicesDe = p.n;
  return p._raices;
}
